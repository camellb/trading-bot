...
PM_SKIP_EXISTING_DAYS   = 0           # TEMP 2026-05-17: forced re-eval every scan so the deep-dive instrumentation actually fires. Revert to 1 once we've captured the BUILT ORDER + api-keys diagnostic.
...
# Sports has been a net loser per archetype so it stays hard-capped;
# the "saved" slots are routed to politics, crypto, geopolitics, etc.
# Total quota = scan budget per tick.
#
# Per-tag value is the MAX markets kept from that tag in a single
# scan AFTER all uncertainty / horizon / liquidity gates are applied.
# A market with multiple top-level tags is fetched per tag bucket but
# deduplicated by id before being returned.
#
# Set to an empty dict to fall back to the legacy untagged
...
PM_SCAN_TAG_QUOTAS: dict[int, int] = {
    ...
    2:      45,   # Politics     (Fed, elections, policy - takes much of the freed budget)
    21:     35,   # Crypto       (best-performing archetype to date - boosted hard)
    100265: 25,   # Geopolitics  (ceasefires, treaties, summits)
    100328: 20,   # Economy      (Fed rates, GDP, jobs)
    596:    10,   # Culture      (Oscars, awards, celebrity)
    101970: 20,   # World        (broader catch-all)
}
