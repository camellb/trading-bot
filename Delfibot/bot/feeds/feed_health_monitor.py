...
warnings in stderr / the dashboard event log. `polymarket` is the only
core feed - if it fails for an extended period we still don't halt
trading (simulation mode has no stop loss to protect), but we do raise
a critical alert so the operator can investigate.
"""
...
class FeedHealthMonitor:
    ...
    def report_healthy(self, feed_name: str) -> None:
        ...
        if prev != "healthy":
            ...
            if not self._in_grace_period() and self._notification_sent.get(feed_name):
                ...
                self._async_dispatch(self._notify_recovered, feed_name)
    ...
    def report_degraded(self, feed_name: str, detail: str,
        ...
        self._async_dispatch(self._notify_degraded, feed_name, detail)
    ...
    # ── Async-dispatch glue ──────────────────────────────────────────────────
    @staticmethod
    def _async_dispatch(coro_fn, *args) -> None:
        try:
    ...
    async def _notify_recovered(feed_name: str) -> None:
        # User-facing feed health is surfaced through the dashboard
        # event-log feed; we just emit a stderr line for the operator.
        print(f"[feed_health] recovered: {feed_name}", file=sys.stderr)
