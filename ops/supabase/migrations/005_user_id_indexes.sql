...
CREATE INDEX IF NOT EXISTS idx_performance_snapshots_user_created
    ON performance_snapshots (user_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_config_change_history_user_created
    ON config_change_history (user_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_event_log_user_created
    ON event_log (user_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_news_event_log_user_created
    ON news_event_log (user_id, created_at DESC);
