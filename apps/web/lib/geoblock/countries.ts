// ISO 3166-1 alpha-2 country list with display name + flag emoji.
//
// Source of truth for the admin "Geoblock" page dropdown and the
// "Not available in your region" page. Keep alphabetical by `name`.
// Flags are the two-regional-indicator emoji pairs, which render as
// flags on every current OS except Windows 10 (falls back to letters).
//
// This list is deliberately inline rather than a npm dependency so we
// do not pull a multi-hundred-KB library into the edge proxy bundle.
...
const CODE_TO_COUNTRY: Map<string, Country> = new Map(
  COUNTRIES.map((c) => [c.code, c] as const),
);

export function countryForCode(code: string | null | undefined): Country | null {
  if (!code) return null;
  return CODE_TO_COUNTRY.get(code.toUpperCase()) ?? null;
}

export function countryNameForCode(code: string | null | undefined): string {
  const c = countryForCode(code);
  if (c) return c.name;
  return (code || "").toUpperCase() || "Unknown";
}

// Convenience for rendering "🇺🇸 United States" style labels.
