...
# One row per learning cycle (fires every LEARNING_CYCLE_TRADE_INTERVAL settled
# trades). Holds both the user-facing text (thesis + deterministic tables +
# footer) and the admin-facing text (adds raw model reasoning excerpts). The
# full deterministic data block is kept in JSONB so future dashboards can
# re-render without re-querying diagnostics. Users read their own reports via
# RLS; admin mirrors select on user_id.
learning_reports = Table(
    ...
    # settled trade count at cycle time - lets the dashboard show cycles
    # chronologically and compute inter-cycle deltas cheaply.
    Column("settled_count", Integer, nullable=False),
    # 2-3 sentence thesis generated by the model from the deterministic data.
    # Kept as plain text so the dashboard can render verbatim.
    Column("thesis", Text, nullable=True),
    # Full user-facing composed message (thesis + tables + footer).
    Column("summary_user", Text, nullable=False),
    # Admin-only version. Adds the model's raw reasoning excerpts for the
    # trades referenced in the report. Never surfaced in user views.
    Column("summary_admin", Text, nullable=True),
    # Deterministic stats block used to compose both texts. Schema:
    #   {"headline":          {n, pnl, roi, win_rate, brier, prev_roi, ...},
    #    "per_archetype":     [{archetype, n, roi, win_rate, brier, verdict}, ...],
    #    "top_wins":          [{market_question, side, pnl, archetype}, ...],
    #    "top_losses":        [{market_question, side, pnl, archetype}, ...],
    #    "calibration":       [{bucket, predicted, observed, n}, ...],
    #    "cost_validation":   {implied, assumed, delta, n},
    #    "proposals":         [{param_name, proposed, evidence, backtest_delta}, ...]}
...
def create_all_tables() -> None:
    ...
    with engine.begin() as conn:
        ...
        # Learning reports - chronological listing per user, fast for the
        # Intelligence → Learning cycles page.
        conn.execute(sa_text(
