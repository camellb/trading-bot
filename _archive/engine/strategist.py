...
class Strategist:
    ...
    async def _execute_enter(self, action: dict, briefing: dict) -> None:
        ...
        # ── Playbook filter (evidence-driven, off by default) ─────────────────
        # See config.ALLOWED_PLAYBOOKS.  When set, skip TRADEs outside the
        # whitelist.  Backtest found momentum is the only positive-EV category
        # on current sample; mean_reversion and breakout are net negative.
        allowed_pbs = getattr(config, "ALLOWED_PLAYBOOKS", None)
        if allowed_pbs:
            pb = (playbook or "").lower()
            if pb not in {p.lower() for p in allowed_pbs}:
                print(
                    f"[strategist] ENTER skipped: playbook '{playbook}' not in "
                    f"ALLOWED_PLAYBOOKS={allowed_pbs}",
                    file=sys.stderr,
                )
                await self.notifier.send(
                    f"ℹ️ <b>Trade filtered</b> — {pair} {direction} "
                    f"playbook <code>{playbook}</code> blocked by config filter."
                )
                return
        ...
        # Pure instrumentation — never raises, never blocks the trading path.
        # Probability = "this trade will close profitable". Resolved by the
        # close path (position_monitor._close_position).
        if trade_id > 0:
            try:
                import calibration
                confidence = action.get("confidence")
                calibration.log_prediction(
                    source        = "crypto_strategist",
                    subject_key   = f"{pair}:trade={trade_id}",
                    probability   = calibration.conviction_to_probability(confidence),
                    category      = playbook,
                    confidence    = confidence,
                    horizon_hours = (time_horizon_days * 24) if time_horizon_days else None,
                    reasoning     = reasoning,
                    metadata      = {
                        "pair":             pair,
                        "direction":        direction,
                ...
                print(f"[strategist] calibration log skipped: {_exc}", file=sys.stderr)
