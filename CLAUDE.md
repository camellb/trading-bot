...
- **The bot owns the entire trade lifecycle. The user never touches Polymarket directly.** If the bot opens a position, it must be able to track, value, and close that position end-to-end. If the bot CANNOT track a market type (multi-outcome / negative-risk / range / scalar / parlay), it MUST refuse to open positions on it at the feed layer (`polymarket_feed._as_market`). User-facing language like "untracked position", "manage on polymarket.com", "manual review required" is BANNED — it exposes bot failure as user homework. When such a situation is detected (only possible if the feed filter has a hole), log to stderr for the engineer; do NOT push to Telegram or dashboard. Fix the filter; don't paper over the leak with user paperwork. Set 2026-05-23 after the Bayern Munich neg-risk leak.
