...
losses = COUNT(pm_positions) WHERE status IN ('settled', 'closed_early')
                               AND realized_pnl_usd < 0
trades = COUNT(pm_positions) WHERE status IN ('settled', 'closed_early')
...
- `invalid` rows are auto-refunded by Polymarket. They are excluded from BOTH the closed-trades count and the win rate (corrected 2026-07-16: the shipped `get_portfolio_stats` has excluded invalid from `settled_total` since 2026-05-24 and every surface consumes that value; the doctrine text previously said "count toward trades", which would have made the next literal reimplementation drift by the number of invalid rows). Invalid counts are reported separately when a surface needs them.
...
5. **Evaluator self-consistency rejection** - `polymarket_evaluator` returns no evaluation when the model's stated reasoning direction contradicts its own probability; the market retries next scan. (The old `force_skip` on `same_event_verified=no` was removed in v1.5.45 per the 2026-06-01 directive - the field still exists on the dataclass but has NO live writer. Doctrine corrected 2026-07-16; do not treat force_skip as an active gate.)
...
- **ROI denominator MUST be pinned. Never recompute `starting = equity - realized - unrealized` on every /api/summary call.** The "identity" `equity = starting + realized + unrealized` is the doctrine, but the right direction to solve it is `starting -> persist, read forever`, NOT `starting <- derive on every read`. Polymarket's PnL fields and our wallet accounting differ by spread + fees on every closed trade; recomputing the denominator means it walks every time a trade closes, climbing ROI as the user LOSES money. 2026-05-25 v1.5.25 tried the derive direction "correctly" and STILL drifted; 2026-05-29 incident: ROI displayed +99.27% at equity $40.54, then +110.38% at equity $38.07 after a $2.47 loss. v1.5.29 fix: column `user_config.live_starting_cash`, snapshotted on the first call where equity + total_pnl are both usable, then read forever. The value never moves again unless the user explicitly rebaselines via Settings. Audit query that MUST always return a stable value: `SELECT live_starting_cash FROM user_config WHERE user_id='local';`. If a future contributor reintroduces an `equity - realized - unrealized` recompute path in `_get_summary`, that's a regression — delete it and read the column. **Pin only on a real wallet read (2026-09-13):** the snapshot is written only when the live overlay succeeded (`data_ready`) and the value is above $1.00; a stored value at or below $1.00 is treated as unset. Before this guard the first live `/api/summary` poll on a cold wallet cache wrote `max(1.0, 0.0)` = $1.00 forever (ROI in the thousands of percent, every risk gate scaled to one dollar).
- **A SELL that didn't fill is NOT a closed trade. Never mark `status='closed_early'` without an actual on-chain fill.** `close_position_early` must refuse the DB UPDATE when `row_mode=='live' AND actual_fill_size is None` and return False so the next exit-policy tick re-evaluates. Without this guard, the bot accepts the SELL's `order_id` response as "done" even when the limit just sits LIVE on the orderbook unfilled — the DB flips to closed, the bot stops managing the position, and the user keeps holding the shares while the market walks away. 2026-05-29 incident: positions #350 (Trump), #353 (Mariners), #359 (Gen.G), #363 (BTC dip) all ghost-closed this way; user flagged #350 with "im losing money cause it should sell at -35% and it's already -50% almost". Companion fix: `_token_has_open_sell` cancels SELLs that have been LIVE >5min with zero matches, so a stale limit price doesn't strand the position forever. **Audit note (corrected 2026-07-16):** the old audit query keyed on `close_tx_hash IS NULL`, but that column is legitimately NULL on confirmed fills (the CLOB status poll often omits `transactionHash` at fill-confirm time), so it false-positives. The enforced invariant is the `actual_fill_size` guard at the code site; audit by checking the guard is intact in `close_position_early` and that `close_reason='stop_loss'/'take_profit'` rows carry a `close_clob_order_id`. Since 2026-07-16 both close UPDATEs also require `AND status='open'` (rowcount 0 = lost a race with settlement = return False, do not overwrite).
...
- **The scan has a time budget; the scheduler fence is a backstop, not the exit.** `pm_analyst.scan_and_analyze` receives `time_budget_s` (210 s from `main.SCAN_TIME_BUDGET_S`) and stops between markets when the next one cannot fit (`engine/scan_budget.TimeBudget`, per-market cap 150 s). A scan that stops on budget is a normal completion: `record_job_ok`, no `trading_blocked` event. The 240 s inner / 260 s outer fences fire only on a real wedge, and the "Market scan failed" alert fires only after two consecutive fence hits. Before 2026-09-13 every tick hit the fence (262 times in two days), every hit pushed "Market scan failed" plus "recovered" to the dashboard and Telegram, and the user read it as "the app keeps timing out". If a fence hit shows up right after a budget-exhausted scan, something synchronous is blocking the job loop: check `job_loop_silence_s` in `/api/health` (the job loop has its own heartbeat, `engine/loop_pulse.LoopPulse`, and the watchdog SIGKILLs at 600 s of silence).
- **No license bypass in the repo or the binary.** The owner bypass token is read from the daemon's `DELFI_OWNER_BYPASS_TOKEN` environment variable only (`Delfibot/install.sh` injects it from `~/.delfi-owner-token` on the dev machine). A hard-coded constant shipped in the public repo until v1.5.84 and unlocked any install for free. `tests/test_license_bypass.py` enforces this.
- **Every settlement UPDATE requires `status='open'` and checks rowcount** (`settle_position` since v1.5.84, `close_position_early` since 2026-07-16). `pm_resolve` and `pm_resolve_fast` overlap; without the guard the same row settled twice, redeemed twice and pushed two WIN/LOSS cards.
- **Provider responses are lists of blocks; read only `type == "text"`.** Current models run adaptive thinking by default, so `response.content[0]` is a thinking block with no `.text`. `engine/llm_client.extract_anthropic_text` joins the text blocks and raises `EmptyLLMResponse` (which fails over to the backup connection) when there are none. `response.content[0].text` anywhere is a regression (2026-09-12: 335 forecasts billed and discarded). Quota, billing, auth and missing-model failures pause that connection for 15 to 30 minutes (`classify_cooldown`); they are never retried per market, and the user-facing alert says which of the four it was and what to do in Settings > Connections.
- **`wallet_address` is derived from the Polymarket private key, never typed.** It is the address the key signs as; the funder (proxy / deposit wallet) is probed separately by `get_poly_signer_info`. `set_user_polymarket_creds` derives it, `heal_wallet_address` repairs installs that hold a key and no wallet, and no UI asks for it. Until v1.5.86 Settings saved the key alone, so the Live switch stayed disabled and `mode=live` was rejected for a field the UI does not have; onboarding's wallet field rejected the deposit address a Polymarket user naturally pastes. A wallet input anywhere in the UI is a regression.
- **The license database is a single point of failure for every buyer; "not found" is not a revoke.** One Postgres (`DATABASE_URL` on the website) sits behind the Stripe webhook (the row is inserted before the key is emailed), the checkout return page, and the desktop's device claim. On 2026-09-18 it was found unreachable (hosted project no longer in DNS, consistent with a free-tier pause after a week idle): a buyer would have been charged with no key and nobody could activate. Health probe that must answer 200: `https://delfibot.com/api/license/check?id=00000000-0000-4000-8000-000000000000`. A daily cron on `/api/cron/keepalive` keeps the database active. The desktop revocation poll wipes the key only when the server returns a non-null `revoked_at`; `valid:false` with "license id not found" is inconclusive, because that is what a rebuilt, empty database answers for every customer. Recovery: `ops/supabase/migrations/README_LICENSE_DB.md`.
